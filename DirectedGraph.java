//Abdullah Waheed
//Graphs - Exercise 1 - Directed Weighted
...
/*	public boolean removeEdge(T begin, T end, double edgeWeight)
	...
	//	VertexInterface<T> beginVertex = vertices.getValue(begin);
		VertexInterface<T> endVertex = vertices.getValue(end);
		DictionaryInterface<T, VertexInterface<T>> beginVertex = vertices.getValue(begin);
		DictionaryInterface<T>, VertexInterface<T>> beginVertex = vertices.getValue(begin);
...
public boolean editProfile(String key,String value)
{
	return vertices.editProfile(key,value);
}*/
	...
	public QueueInterface<T> getBreadthFirstTraversal(T origin)
	{
		...
		QueueInterface<T> mutual = new LinkedQueue<T>();
		Boolean check;
		int count=0;

		originVertex.visit();//TRUE
		...
		while (!vertexQueue.isEmpty())
		{
			...
			while (neighbors.hasNext())
			{
				...
				if (!nextNeighbor.isVisited()) {
					...
					check = hasEdge(origin, nextNeighbor.getLabel());
					if (check)
					{
						System.out.println(nextNeighbor.getLabel() + " Friend");
						mutual.enqueue(nextNeighbor.getLabel());
						count++;
					}
					else {
						for (int i = 0; i < count; i++)
						{
							T temp = mutual.dequeue();
							check = hasEdge(temp, nextNeighbor.getLabel());
							if(check)
								System.out.println(nextNeighbor.getLabel() + " Mutual with  " +temp);
							mutual.enqueue(temp);
						}
					}
